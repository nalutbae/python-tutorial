...
- [사전 준비](#-사전-준비)
   - [Python 설치 확인](#1-python-설치-확인)
   - [Jupyter 설치](#2-jupyter-설치)
   - [Jupyter 실행하기](#3-jupyter-실행하기)
- [노트북 사용법](#-노트북-사용법)
- [커리큘럼](#-커리큘럼)
   - [1.변수](#01-변수--파이썬-기초)
   - [2.제어](#02-제어--흐름-제어)
   - [3.함수](#03-함수--함수와-고급-기법)
   - [4.입출력](#04-입출력--파일과-환경)
   - [5.클래스 및 객체 지향 프로그래밍](#05-클래스--객체-지향-프로그래밍)
   - [6.모듈](#06-모듈--표준-라이브러리)
   - [7.표준 라이브러리](#07-표준라이브러리--표준-라이브러리-심화)
...
| # | 노트북 | 주제 | 내용 요약 |
|---|--------|------|----------|
| 1 | `1_NumPy_소개와_배열기초.ipynb` | NumPy 소개와 배열 기초 | ndarray, 배열 생성, 인덱싱/슬라이싱, 불리언 인덱싱 |
| 2 | `2_배열_연산과_형태변환.ipynb` | 배열 연산과 형태 변환 | 벡터화 연산, 브로드캐스팅, reshape, 전치, 결합/분할 |
...
| 5 | `5_표본추출과_모집단.ipynb` | 표본 추출과 모집단 | 모집단/표본, 무작위추출, 난수시드, 모집단분포, 정규분포, 표본평균 수렴 |
| 6 | `6_표본통계량의성질.ipynb` | 표본 통계량의 성질 | 대수법칙, 표본평균<모표준편차, 표준오차, 표본분산 편향, 불편분산, 중심극한정리 |
| 7 | `7_정규분포와_응용.ipynb` | 정규분포와 응용 | PDF, CDF, ppf/신뢰구간, t값, t분포 |
| 8 | `8_통계적_추정.ipynb` | 통계적 추정 | 점추정, 구간추정, 신뢰구간, 신뢰구간 폭 요소, 시뮬레이션 |
...
| # | 노트북 | 주제 | 내용 요약 |
|---|--------|------|----------|
| 1 | `1_OpenCV_소개와_이미지기초.ipynb` | OpenCV 소개와 이미지 기초 | 설치, 이미지 읽기/저장, 픽셀/채널, 색상 공간 변환 |
| 2 | `2_OpenCV_이미지_처리기법.ipynb` | 이미지 처리 기법 | 리사이징, 블러/샤프닝, 임계값, Canny 엣지, 모폴로지 |
| 3 | `3_OpenCV_색상검출과_도형그리기.ipynb` | 색상 검출과 도형 그리기 | HSV 색상 검출, 마스크, 도형/텍스트, 윤곽선 분석 |
| 4 | `4_OpenCV_기하학적_변환.ipynb` | 기하학적 변환 | 확대/축소, 회전, 대칭, 아핀/투시 변환 |
| 5 | `5_OpenCV_실전_프로젝트.ipynb` | 실전 프로젝트 | 워터마크, PSNR 품질 측정, 처리 파이프라인 |
| 6 | `6_Pillow_소개와_기본조작.ipynb` | Pillow 소개와 기본 조작 | 이미지 열기/저장, 크기 조정, 자르기, 회전, 색상 모드, EXIF |
| 7 | `7_Pillow_필터와_이미지효과.ipynb` | Pillow 필터와 이미지 효과 | 내장 필터, 향상(밝기/대비/색상), 합성, 한글 텍스트 |
| 8 | `8_Pillow_실전_응용.ipynb` | Pillow 실전 응용 | 썸네일, 워터마크, 포맷 변환/최적화, PIL↔NumPy↔OpenCV 연동 |
...
│   ├── cov.csv
│   ├── fish_multi_2.csv
│   ├── fish_length_100000.csv
│   ├── fish_length.csv
│   ├── junk_food_weight.csv
│   ├── paired_t_test.csv
│   ├── click_data.csv
...
│   ├── 3_OpenCV_색상검출과_도형그리기.ipynb
│   ├── 4_OpenCV_기하학적_변환.ipynb
│   ├── 5_OpenCV_실전_프로젝트.ipynb
│   ├── 6_Pillow_소개와_기본조작.ipynb
│   ├── 7_Pillow_필터와_이미지효과.ipynb
...
8. **[08]Numpy**를 순서대로 완료 (배열기초 → 연산/형태변환 → 통계/수학함수 → 선형대수/난수 → 실전응용)
9. **[11]통계**를 순서대로 완료 (변량데이터/기초통계량 → 다변량데이터/pandas → 공분산/상관계수 → 데이터시각화 → 표본추출/모집단 → 표본통계량성질 → 정규분포/응용 → 통계적추정 → 가설검정 → 평균값차이검정 → 분할표검정)
